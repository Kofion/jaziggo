...
if ($null -eq $Options) {
    # ConvertFrom-Yaml is not included with Windows PowerShell. Parse the extension's
    # small, fixed config shape without requiring Python or an additional YAML module.
    try {
        $simpleOptions = @{}
        $simpleMarkers = @{}
        foreach ($line in Get-Content -LiteralPath $ExtConfig) {
            if ($line -match '^\s*context_file\s*:\s*(.+?)\s*$') {
                $simpleOptions['context_file'] = $Matches[1].Trim().Trim('''', '"')
            } elseif ($line -match '^\s+start\s*:\s*(.+?)\s*$') {
                $simpleMarkers['start'] = $Matches[1].Trim().Trim('''', '"')
            } elseif ($line -match '^\s+end\s*:\s*(.+?)\s*$') {
                $simpleMarkers['end'] = $Matches[1].Trim().Trim('''', '"')
            }
        }
        if ($simpleOptions.ContainsKey('context_file')) {
            if ($simpleMarkers.Count -gt 0) {
                $simpleOptions['context_markers'] = $simpleMarkers
            }
...
if ($null -eq $Options) {
    # Fall back to Python+PyYAML for config shapes outside the built-in subset.
    $pythonCmd = $null
...
if (-not $PlanPath) {
    ...
    try {
        ...
        if ($candidate) {
            ...
            if ($candidate.FullName.StartsWith($rootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
                $PlanPath = $candidate.FullName.Substring($rootPrefix.Length).Replace('\','/')
            }
        }
    } catch {
