...
type EndResponsibleLinkDialogProps = Readonly<{
  open: boolean
...
function errorMessageForResponse(body: ApiEnvelope<ResponsibleLinkDto> | null) {
  if (body?.success === false) {
    ...
    if (body.error.code === DOMAIN_ERROR_CODE.NOT_FOUND) {
      return "Vínculo administrativo não encontrado."
    }
...
export function EndResponsibleLinkDialog({
  open,
  responsibleLinkId,
  onOpenChange,
  onSuccess,
}: EndResponsibleLinkDialogProps) {
  const formId = useId()
  const errorId = useId()
  ...
  async function handleConfirm() {
    ...
    if (responsibleLinkId.trim().length === 0) {
      setSuccessMessage(null)
    ...
    try {
      const response = await fetch(
        `/api/v1/responsible-links/${encodeURIComponent(responsibleLinkId)}/end`,
        {
          method: "PATCH",
          headers: {
      ...
      const body = await readJsonEnvelope<ResponsibleLinkDto>(response)
  ...
  return (
    <ConfirmDialog
      ...
      description="Registre o encerramento histórico do vínculo ativo sem apagar o histórico administrativo."
      intent="danger"
    ...
    >
      <div className="space-y-4">
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          O encerramento altera o status do vínculo para histórico. Depois disso, um novo vínculo pode ser criado com os dados corretos.
        </div>
