...
interface BurialLinksByDeceasedRouteContext {
...
export async function GET(
  ...
  context: BurialLinksByDeceasedRouteContext,
) {
  const requestId = crypto.randomUUID()

  try {
    const { deceasedId } = await context.params
    const burialLinks = await listBurialLinksByDeceased(deceasedId)
